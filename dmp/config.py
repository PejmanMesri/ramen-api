...
def _to_async_dsn(url: str) -> str:
    ...
    if not u.startswith("postgresql+asyncpg://") and u.startswith("postgresql://"):
        u = "postgresql+asyncpg://" + u[len("postgresql://"):]
    # asyncpg rejects unknown query params — drop ones Neon DSNs carry
    # (`channel_binding`) while keeping sslmode, which asyncpg understands.
    if "?" in u:
        base, query = u.split("?", 1)
        kept = [p for p in query.split("&") if p and not p.startswith("channel_binding=")]
        # SQLAlchemy's asyncpg dialect takes `ssl=`, not `sslmode=`.
        kept = [p.replace("sslmode=", "ssl=", 1) if p.startswith("sslmode=") else p for p in kept]
...
class Settings(BaseSettings):
    ...
    # How often the job re-verifies pending gateway payments (seconds).
    payment_reconcile_interval_seconds: int = 300
    # A pending payment older than this (minutes) does a final verify and then
    # is marked Failed. With the default 5-min interval that gives natural
    # re-checks at ~5/10/15 min before the final one.
    payment_pending_fail_after_minutes: int = 20
