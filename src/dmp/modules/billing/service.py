...
class BillingService:
    ...
    async def handle_callback(
        ...
        await self._finalize_success(session, payment, ref_id, card_pan)
        return _ok(payment)
    ...
    async def _finalize_success(
        ...
        """Mark a payment Succeeded and activate/extend its subscription.
        Shared by the gateway callback and the pending-payment reconciler."""
        from datetime import timedelta
    ...
    # ── Pending-payment reconciler (background job) ───────────────────────────

    async def reconcile_pending_payments(self, session: AsyncSession, fail_after_minutes: int) -> dict:
        """Re-verify PENDING gateway payments so abandoned checkouts finalise
        without the browser ever returning to /payment-result.

        Policy (see PAYMENT_RECONCILE_* in config):
          - the job runs every PAYMENT_RECONCILE_INTERVAL_SECONDS (default 5 min),
            so a pending payment is naturally re-checked at ~5/10/15 min of age
          - a payment still unverified at PAYMENT_PENDING_FAIL_AFTER_MINUTES
            (default 20 min) does one final verify and is then marked Failed
            (its pending subscription is cancelled)
          - Failed/Succeeded/Cancelled payments are never re-checked
        """
        from datetime import timedelta

        cutoff = utcnow() - timedelta(minutes=fail_after_minutes)
        stmt = (
            select(Payment)
            .options(selectinload(Payment.subscription), selectinload(Payment.plan))
            .where(Payment.status == PaymentStatus.Pending)
            .order_by(Payment.created_at)
        )
        pendings = (await session.execute(stmt)).scalars().all()
        result = {"checked": len(pendings), "succeeded": 0, "failed": 0, "stillPending": 0}

        async with httpx.AsyncClient(base_url=zarinpal._base_url(), timeout=30.0) as client:
            for payment in pendings:
                if not payment.authority:
                    # No authority → nothing to verify; fail it if past cutoff.
                    ...
                    continue
                verified, _code, ref_id, card_pan = await zarinpal.verify(
                    client, payment.amount_toman, payment.authority
                )
                if verified:
                    await self._finalize_success(session, payment, ref_id, card_pan)
                    result["succeeded"] += 1
                elif payment.created_at and payment.created_at <= cutoff:
                    payment.status = PaymentStatus.Failed
                    if payment.subscription and payment.subscription.status == SubscriptionStatus.PendingPayment:
                        payment.subscription.status = SubscriptionStatus.Cancelled
                    result["failed"] += 1
                else:
                    result["stillPending"] += 1  # retried on the next tick

        await session.commit()
        if result["checked"]:
            log.info("[Reconciler] %s", result)
        return result
    ...
    async def admin_stats(self, session: AsyncSession) -> dict:
        ...
        recent_stmt = (
            ...
            .where(Payment.status != PaymentStatus.Failed)  # failed never shows on the dashboard
            .order_by(Payment.created_at.desc())
            .limit(10)
        )
    ...
    async def get_entitlement(self, session: AsyncSession, user_id: str) -> dict:
        """Active tier derived from the active subscription's plan features."""
        now = utcnow()
        stmt = (
            select(Subscription)
        ...
        if sub is None or sub.plan is None:
            return {"tier": "free", "planName": None, "endAt": None}
        features = sub.plan.features if isinstance(sub.plan.features, dict) else {}
        tier = str(features.get("tier") or "gold").lower()
        if tier not in ("gold", "platinum"):
            tier = "gold"
        return {
            "tier": tier,
            "planName": sub.plan.name or sub.plan.name_key,
            "planKey": sub.plan.name_key,
            "endAt": sub.end_at.isoformat() if sub.end_at else None,
        }
