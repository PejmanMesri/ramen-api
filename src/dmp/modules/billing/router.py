...
@router_client.get("/entitlement")
async def entitlement(
    user_id: str = Depends(current_user_id),
    session: AsyncSession = Depends(get_session),
):
    """The user's current tier: free | gold | platinum (from the active
    subscription's plan metadata)."""
    return ok(await _billing.get_entitlement(session, user_id))
...
@router_admin.post("/payments/reconcile")
async def reconcile_pending_payments(
    _admin: Admin = Depends(current_admin),
    session: AsyncSession = Depends(get_session),
):
    """Run the pending-payment reconciler on demand (same logic as the
    background job: verify pendings, fail the stale ones)."""
    from ...config import get_settings

    result = await _billing.reconcile_pending_payments(
        session, get_settings().payment_pending_fail_after_minutes
    )
    return ok(result)
