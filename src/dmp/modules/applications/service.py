...
# Legacy wire values (pre-2026-09 kanban rework) still accepted on input.
_LEGACY_STATUS_ALIASES: dict[str, ApplicationStatus] = {
    "draft": ApplicationStatus.Preparing,
    "submitted": ApplicationStatus.Applied,
    "underreview": ApplicationStatus.Waiting,
...
class ApplicationService:
    ...
    async def update(
        ...
        app = await self._load(session, app_id, user_id=user_id)
        # The kanban lets owners drag a card to any column, so status is always
        # settable; only the descriptive fields lock once a final state is set.
        if app.status in (ApplicationStatus.Accepted, ApplicationStatus.Rejected) and req.status is None:
