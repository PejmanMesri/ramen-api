...
from .country_infer import infer_country_code
from .mapper import (
    AdmissionPayload,
    ProgramPayload,
    RankingPayload,
...
DESC_SOURCES = [
    # (overview field, source key, display label)
    ("about_university", "qs", "QS"),
    ("introduction", "shanghai", "Shanghai Ranking"),
    ("summary", "us_news", "US News"),
]
WEBSITE_SOURCE_KEYS = ("qs", "shanghai", "us_news")


def _collect_descriptions(json_content: dict) -> list[dict]:
    """Every non-empty description across the dump's overview fields, tagged
    with its ranking source (QS / Shanghai / US News)."""
    out: list[dict] = []
    seen: set[str] = set()
    overview = json_content.get("university_overview")
    if not isinstance(overview, dict):
        return out
    for field, src, label in DESC_SOURCES:
        node = overview.get(field)
        text = node.get(src) if isinstance(node, dict) else node if isinstance(node, str) else None
...
def _pick_website(json_content: dict) -> str | None:
    ident = json_content.get("university_identity")
    if not isinstance(ident, dict):
        return None
    node = ident.get("website")
    if isinstance(node, str) and node.strip():
        return node.strip()
    if isinstance(node, dict):
        for key in WEBSITE_SOURCE_KEYS:
            v = node.get(key)
...
def _collect_subject_rankings(json_content: dict) -> list[dict]:
    """Best-ranked subjects across sources (deduped): rankings.subject.*."""
    out: list[dict] = []
    seen: set[str] = set()

    def add(subject: str, rank: str) -> None:
        key = subject.strip().lower()
        if not key or key in seen or not str(rank).strip():
            return
        seen.add(key)
        out.append({"subject": subject.strip(), "rank": str(rank).strip()})

    subject_root = _pick(json_content, "rankings", "subject")
    if isinstance(subject_root, dict):
        # shanghai / us_news: best_ranked_subjects arrays
        for src in ("shanghai", "us_news"):
            node = subject_root.get(src)
            if not isinstance(node, dict):
                continue
            best = node.get("best_ranked_subjects")
            if isinstance(best, list):
                for item in best:
                    if isinstance(item, dict):
                        add(str(item.get("subject_name") or ""), str(item.get("rank") or ""))
        # us_news subjects carry overall_score instead of rank
        us = subject_root.get("us_news")
        if isinstance(us, dict) and not any(True for _ in out):
            for item in us.get("subjects") or []:
                if isinstance(item, dict) and item.get("subject_name"):
                    add(str(item["subject_name"]), str(item.get("overall_score") or ""))
    return out[:30]


def _collect_rank_history(json_content: dict) -> dict | None:
    """QS historical global ranks: rankings.global.qs.historical_rank_data."""
    hist = _pick(json_content, "rankings", "global", "qs", "historical_rank_data")
    if not isinstance(hist, dict):
        return None
    out = {str(k): str(v) for k, v in sorted(hist.items()) if str(v).strip()}
    return out or None
...
def map_dump_payload(json_content: dict, fallback_name: str | None) -> UniversityPayload | None:
    ...
    payload = UniversityPayload(
        ...
        or "",
        website=_pick_website(json_content),
        descriptions=_collect_descriptions(json_content),
        subject_rankings=_collect_subject_rankings(json_content),
        rank_history=_collect_rank_history(json_content),
        ...
        # The dump's country fields are sparse (present for ~1/3 of rows) while
        # the location/address strings usually embed the country. Derive it so
        # fresh-PC imports fill universities.country_id on their own.
        or infer_country_code(
            _pick_str(json_content, ("university_identity", "location", "us_news")),
            _pick_str(json_content, ("university_identity", "location", "shanghai")),
            _pick_str(json_content, ("university_identity", "location", "qs")),
            _pick_str(json_content, ("university_identity", "address", "qs")),
            _pick_str(json_content, ("university_identity", "address", "us_news")),
            _pick_str(json_content, ("university_identity", "address", "shanghai")),
            _pick_str(json_content, ("university_overview", "campus_location", "qs")),
            fallback_name,
        ),
