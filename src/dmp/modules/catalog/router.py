...
async def list_public(
    ...
    data = await _catalog.list_public_cards(session, search, countryId, page, limit, sort, order)
    return ok(data)
...
@router_public.get("/stats")
async def public_stats(session: AsyncSession = Depends(get_session)):
    """Real counts for the marketing pages (universities, countries, users, applications)."""
    from sqlalchemy import func, select

    from ...domain.models import Application, Country, University, User

    universities = (await session.execute(
        select(func.count(University.id)).where(University.is_published.is_(True))
    )).scalar_one()
    ...
    applications = (await session.execute(select(func.count(Application.id)))).scalar_one()
    return ok({"universities": universities, "countries": countries, "users": users, "applications": applications})


@router_public.get("/plans")
async def public_plans(session: AsyncSession = Depends(get_session)):
    """Active subscription plans (anonymous — shown on the landing pricing)."""
    from ..billing.service import BillingService

    plans = await BillingService().list_active_plans(session)
    return ok([p.model_dump(exclude_none=True) for p in plans])
