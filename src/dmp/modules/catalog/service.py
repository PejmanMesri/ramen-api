...
from sqlalchemy import Integer, asc, cast, delete, desc, func, select
...
class CatalogService:
    ...
    async def list_public_cards(
        ...
        # Ordering: name (default) | rank (numeric prefix of qs_world_rank,
        # e.g. "#5", "100-150" → 5, 100) | country (joined name).
        direction = desc if (order or "asc").lower() == "desc" else asc
        if sort == "rank":
            rank_num = cast(
                func.nullif(func.regexp_replace(func.coalesce(University.qs_world_rank, ""), "[^0-9].*", ""), ""),
                Integer,
            )
            stmt = stmt.order_by(direction(rank_num).nulls_last(), University.name)
        elif sort == "country":
            stmt = stmt.order_by(direction(Country.name).nulls_last(), University.name)
        else:
            stmt = stmt.order_by(direction(func.lower(University.name)))

        stmt = stmt.offset((p - 1) * lim).limit(lim)
    ...
    async def import_university_data(self, session: AsyncSession, refresh: bool = True) -> dict:
        """Import rows from the restored `university_data` landing table into the
        canonical catalog.

        Semantics (safe to re-run and future-dump tolerant):
        - New universities are created.
        - refresh=True (default): existing rows are REFRESHED — dump-mapped
          fields are overwritten with the latest dump data (ids/slugs survive
          so applications keep working); children replaced with fresh data.
        - refresh=False: fill-empty merge that never clobbers manual edits.
        """
        from . import dump_mapper
        ...
        for row in rows:
            ...
            if slug in seen_slugs:
                summary["skipped"] += 1  # duplicate variant of the same university
                continue
            seen_slugs.add(slug)
            existing = (
                await session.execute(
            ...
            if refresh:
                # Overwrite dump-mapped fields with the freshest data.
                if payload.about:
                    u.about = payload.about
                if payload.qs_world_rank:
                    u.qs_world_rank = payload.qs_world_rank
                if payload.campus_location:
                    u.campus_location = payload.campus_location
                if payload.website:
                    u.website = payload.website
                if payload.descriptions:
                    u.descriptions = payload.descriptions
                if payload.subject_rankings:
                    u.subject_rankings = payload.subject_rankings
                if payload.rank_history:
                    u.rank_history = payload.rank_history
                if payload.facilities:
                    u.facilities = payload.facilities
                if payload.scholarships:
                    u.scholarships = payload.scholarships
                if payload.career_services:
                    u.career_services = payload.career_services
                if payload.overall_tuition:
                    u.tuition_fees = payload.overall_tuition
                if payload.student_life is not None:
                    u.student_life = payload.student_life
            else:
                ...
                if not u.campus_location and payload.campus_location:
                    u.campus_location = payload.campus_location
                if not u.website and payload.website:
                    u.website = payload.website
                if payload.descriptions:
                    have = {(d.get("source"), (d.get("text") or "").strip().lower()) for d in (u.descriptions or [])}
                    for d in payload.descriptions:
                        key = (d.get("source"), (d.get("text") or "").strip().lower())
                        if key not in have and (d.get("text") or "").strip():
                            u.descriptions = [*(u.descriptions or []), d]
                if not u.subject_rankings and payload.subject_rankings:
                    u.subject_rankings = payload.subject_rankings
                if not u.rank_history and payload.rank_history:
                    u.rank_history = payload.rank_history
            ...
            if (summary["created"] + summary["merged"]) % 100 == 99:
                await session.commit()
            elif created:
                await session.commit()  # new id must exist before children
            ...
            if created or refresh:
                # (children of a brand-new row are empty by definition; never
                # touch relationships on pending instances — async lazy load)
                if payload.programs or (not created and u.programs):
                    await self._replace_programs(session, u.id, payload.programs)
                if payload.admissions or (not created and u.admissions):
                    await self._replace_admissions(session, u.id, payload.admissions)
                if payload.student_staff_buckets or (not created and u.student_staff):
                    await self._upsert_student_staff(session, u.id, payload.student_staff_buckets, payload.student_life)
                if payload.ranking or (not created and u.ranking):
    ...
    async def _resolve_country_id_by_name(self, session: AsyncSession, name: str) -> str | None:
        """Country lookup for dump payloads. Accepts the English name
        ('United States'), an ISO alpha-2 code from country_infer ('CA'), or a
        common alias ('Russia' → Russian Federation row); prefix fallback last."""
        value = name.strip()
        if not value:
            return None
        if len(value) == 2 and value.isalpha() and value.isupper():
            by_code = (
                await session.execute(select(Country).where(Country.code == value))
            ).scalar_one_or_none()
            if by_code:
                return by_code.id
        exact = (
            await session.execute(select(Country).where(func.lower(Country.name) == value.lower()))
        ).scalar_one_or_none()
        if exact:
            return exact.id
        fuzzy = (
            await session.execute(
                select(Country)
                .where(func.lower(Country.name).like(f"%{value.lower()}%"))
                .limit(1)
            )
        ).scalar_one_or_none()
...
def _to_detail(u: University) -> UniversityDetail:
    ...
    return UniversityDetail(
        ...
        subjectRankings=u.subject_rankings,
        rankHistory=u.rank_history,
