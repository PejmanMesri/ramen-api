...
from .modules.bookmarks.router import router as bookmarks_client
...
async def lifespan(app: FastAPI):
    ...
    # Background job: re-verify pending gateway payments (abandoned checkouts)
    # every interval; stale ones are failed after the configured window.
    reconcile_task = asyncio.create_task(_payment_reconciler_loop(settings))
...
async def _payment_reconciler_loop(settings) -> None:
    """Every PAYMENT_RECONCILE_INTERVAL_SECONDS, re-verify Pending payments.
    Successful verifies activate the subscription; payments still pending after
    PAYMENT_PENDING_FAIL_AFTER_MINUTES are marked Failed (never re-checked)."""
    from .modules.billing.service import BillingService

    interval = max(60, settings.payment_reconcile_interval_seconds)
    fail_after = settings.payment_pending_fail_after_minutes
    billing = BillingService()
    log.info("[Reconciler] started (every %ss, fail after %smin)", interval, fail_after)
    # Small delay so startup (migrations/seeding) finishes first.
    await asyncio.sleep(20)
    while True:
        try:
            async with get_session_factory()() as session:
                await billing.reconcile_pending_payments(session, fail_after)
...
async def _run_migrations() -> None:
    ...
    Runs as a subprocess — in-thread alembic deadlocks with the uvicorn
    reloader's executor after docker/host restarts."""
    import subprocess
    import sys
    ...
    if proc.returncode != 0:
        raise RuntimeError("alembic upgrade failed: " + out.decode(errors="replace")[-2000:])
...
def create_app() -> FastAPI:
    ...
    app.include_router(bookmarks_client)
