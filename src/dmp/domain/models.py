...
class UserBookmark(Base):
    """Saved items (universities etc.) shown in the user's bookmarks panel."""

    __tablename__ = "user_bookmarks"
    __table_args__ = (
        UniqueConstraint("user_id", "item_type", "item_id", name="uq_user_bookmark_item"),
    )
    ...
    item_type: Mapped[str] = mapped_column(String(32), nullable=False, default="university")
...
class University(Base):
    ...
    # [{"source": "QS", "text": "..."}] — every dump description, per source
    descriptions: Mapped[list | None] = mapped_column(JSONB)
    # best ranked subjects from the dump: [{"subject": "...", "rank": "76-100"}]
    subject_rankings: Mapped[list | None] = mapped_column(JSONB)
    # {"2015": "701+", ..., "2027": "1001-1200"} — QS historical global rank
    rank_history: Mapped[dict | None] = mapped_column(JSONB)
...
class Application(Base):
    ...
    status: Mapped[ApplicationStatus] = mapped_column(
        String(32), nullable=False, default=ApplicationStatus.Preparing, index=True
    )
